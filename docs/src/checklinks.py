...
if missing_file:
    print "Archivos linkados en %s pero no se encuentran:" % (
        os.path.basename(ref),)
    for i in sorted(missing_file):
        print "\t%r" % i
if missing_anchor:
    print "Anclajes usados en %s pero no definidos en archivo linkado:" % (
        os.path.basename(ref),)
    for i in sorted(missing_anchor):
        print "\t%r" % i
if unlisted_targets:
    print "Links a archivos no listados como objetivos:"
...
if missing_file:
    print "Archivos linkados en %s pero no se pudieron encontrar:" % (
        os.path.basename(ref),)
    for i in sorted(missing_file):
        print "\t%r" % i
if missing_anchor:
    print "Anclajes utilizados en %s pero no definidos en el archivo linkado:" % (
        os.path.basename(ref),)
    for i in sorted(missing_anchor):
        print "\t%r" % i
if unlisted_targets:
    print "Enlaces a archivos que no figuran como targets:"
    for i in sorted(unlisted_targets):
        print "\t%r" % i
    print "Si todos los objetivos de link no están listados en el Submakefile, entonces los resultados de este programa no son confiables."
print "links buenos: %d/%d" % (len(good), len(refs))
